                        ...
                        if(response.status == 400) {
                          return "FAILURE";
                            ...
                            if(body){ 
                                ...
                                if(key == 'batch'){
                                    this.group.get_my_batches(body);
                                }
                                // fetch topic by pagination
                                if(key == 'topic'){
                                    let paged = this.profileTabPaged[key]?this.profileTabPaged[key]:1;
                                    this.forum.get_my_topics(body,paged);
                                }
                                // fetch topic by pagination
                                if(key == 'reply'){
                                    let paged = this.profileTabPaged[key]?this.profileTabPaged[key]:1;
                                    this.forum.get_my_replies(body,paged);
                                }
